...
var bootstrap = function ($, learun) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $("#num").hide();
            if (!isLook) {
                $('#F_FileContent').DocUploader({ upLoad: true });
                $('#F_FileContent_New').DocUploader({ upLoad: true });
                $('#F_Attachments').DocUploader({ upLoad: true, defaultUpload: true, extensions: '*' });
            } else {
    ...
    validForm = function () {
        ...
        var filename = $('#F_FileContent').find('.lrUploader-input').text()
        if (!filename || filename.length <= 0) {
            learun.alert.warning('必须上传正文')
            return false;
        }
        return true;
    };

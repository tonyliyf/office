 ...
 * 创建人：东雅图-前端开发组
 * 日 期：2018.7.12
 * 描 述：东雅图移动端框架(ADMS) 流程我的任务
 */
(function () {
    var begin = '';
    var end = '';
    var $header = null;
    learun.storage.set('CurrentAuditPage', 'nworkflow/myaudit')
    var page = {
        currentPage: 0,
        grid: [],
        init: function ($page) {
            ...
            $header.find('.lr-form-header-more').on('tap', function () {
                learun.nav.go({ path: 'search', title: '', isBack: true, isHead: true, param: 'nworkflow/myaudit' });// 告诉搜索页本身所在的地址
            });
            $page.find('#myaudit_tab').toptab(['待办', '已办'], function (_index) {
            ...
            //var s = $page.find('#myaudit_tab')
            //s = s.find('div.f-toptab-btns')
            //s = s.find('div')
            //s = s.find('a[data-value="1"]')
            //s.tap()
        },
        lclass: 'lr-list lr-flow-list',
        loadData: function (param, callback, $page) {// 列表加载后台数据
            ...
            var url = '';
            var mypage = learun.nav.getpage('nworkflow/myaudit');
            switch (mypage.currentPage) {
                //case 0:
                //    url = config.webapi + "learun/adms/newwf/mylist";
                //    break;
                case 0:
                    url = config.webapi + "learun/adms/newwf/mytask?type=0";
                    break;
                case 1:
                    url = config.webapi + "learun/adms/newwf/mytask?type=1";
                    break;
            }
        ...
        rowRender: function (_index, _item, _$item, $page) {// 渲染列表行数据
            ...
            if (_item.F_IsAgain === 1 && mypage.currentPage !== 1) {
                statusText = '<span style="color:red;" >重新发起</span>';
            }

            if (_item.F_TaskType == 3 && mypage.currentPage === 0) {
                statusText = '【加签】' + statusText;
            }

            if (_item.F_IsUrge == "1" && mypage.currentPage === 0) {

...
namespace Learun.Application.Web.Areas.AssetManager.Controllers
{
    ...
    /// </summary>
    public class DC_ASSETS_EquipmentPartsInfoController : MvcControllerBase
    {
        private DC_ASSETS_EquipmentPartsInfoIBLL dC_ASSETS_EquipmentPartsInfoIBLL = new DC_ASSETS_EquipmentPartsInfoBLL();
        ...
        public ActionResult GetPageList(string pagination, string queryJson)
        {
            ...
            var data = dC_ASSETS_EquipmentPartsInfoIBLL.GetPageList(paginationobj, queryJson);
            var jsonData = new
            {
                rows = data,
        ...
        public void ExportExcel(string queryJson, string fileName, string exportField, string columnJson)
        {
            ...
            var dt = dC_ASSETS_EquipmentPartsInfoIBLL.ExportData(queryJson);
            //行数据
        ...
        {
            string relativeDevice;
            var DC_ASSETS_EquipmentPartsInfoData = dC_ASSETS_EquipmentPartsInfoIBLL.GetDC_ASSETS_EquipmentPartsInfoEntity(keyValue, out relativeDevice);
            var jsonData = new
            {
                DC_ASSETS_EquipmentPartsInfo = DC_ASSETS_EquipmentPartsInfoData,
        ...
        {
            dC_ASSETS_EquipmentPartsInfoIBLL.DeleteEntity(keyValue);
            return Success("删除成功！");
        }
        /// <summary>
        /// 保存实体数据（新增、修改）
        ...
        [AjaxOnly]
        public ActionResult SaveForm(string keyValue, string strEntity, string RelativeDevice)
        {
            if (string.IsNullOrEmpty(keyValue))
            {
                iBll.UseRuleSeed("10016");
            }
            DC_ASSETS_EquipmentPartsInfoEntity entity = strEntity.ToObject<DC_ASSETS_EquipmentPartsInfoEntity>();
            dC_ASSETS_EquipmentPartsInfoIBLL.SaveEntity(keyValue, entity, RelativeDevice);
            return Success("保存成功！");
        }
        #endregion

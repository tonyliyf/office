        ...
        public OperatorResult IsOnLine(string token, string loginMark,string account = "")
        {
            ...
            try
            {
                ...
                if (operatorInfo != null && operatorInfo.token == token)
                {
                    ...
                    if (span.TotalHours >= 72)// 登录操作过12小时移除
                    {
                    ...
                    else
                    {
                        ...
                        if (userEntity != null)
                        {
                            ...
                            var roleList = roleBLL.GetRoleList(userEntity.F_UserId);
                            RoleEntity role1 = roleBLL.GetRole("集团高层领导");
                            RoleEntity role2 = roleBLL.GetRole("集团分管投资领导");
                            RoleEntity role21 = roleBLL.GetRole("集团分管市场领导");
                            RoleEntity role22 = roleBLL.GetRole("集团分管财务领导");
                            RoleEntity role23 = roleBLL.GetRole("集团分管工程领导");
                            RoleEntity role3 = roleBLL.GetRole("集团部长");
                            RoleEntity role4 = roleBLL.GetRole("分子公司领导");

                            if (role1 != null && roleBLL.IsExit(userEntity.F_UserId, role1.F_RoleId))
                            {
                               userInfo.F_Level = 5;

                            }
                            else if (role2 != null && roleBLL.IsExit(userEntity.F_UserId, role2.F_RoleId))
                            {
                                userInfo.ManageDeptIds = GetSplit(role2.F_ManageDeptIds);
                                userInfo.ManageCompanyIds = GetSplit(role2.F_ManageCompanyIds);
                                userInfo.F_Level = 4;

                            }
                            else if (role21 != null && roleBLL.IsExit(userEntity.F_UserId, role21.F_RoleId))
                            {
                                userInfo.ManageDeptIds = GetSplit(role21.F_ManageDeptIds);
                                userInfo.ManageCompanyIds = GetSplit(role21.F_ManageCompanyIds);
                                userInfo.F_Level = 4;

                            }
                            else if (role22 != null && roleBLL.IsExit(userEntity.F_UserId, role22.F_RoleId))
                            {
                                userInfo.ManageDeptIds = GetSplit(role22.F_ManageDeptIds);
                                userInfo.ManageCompanyIds = GetSplit(role22.F_ManageCompanyIds);
                                userInfo.F_Level = 4;

                            }
                            else if (role23 != null && roleBLL.IsExit(userEntity.F_UserId, role23.F_RoleId))
                            {
                                userInfo.ManageDeptIds = GetSplit(role23.F_ManageDeptIds);
                                userInfo.ManageCompanyIds = GetSplit(role23.F_ManageCompanyIds);
                                userInfo.F_Level = 4;

                            }
                            else if (role3 != null && roleBLL.IsExit(userEntity.F_UserId, role3.F_RoleId))
                            {
                                
                                userInfo.ManageDeptIds = userInfo.departmentId;
                                userInfo.F_Level = 2;

                            }
                            else if (role4 != null && roleBLL.IsExit(userEntity.F_UserId, role4.F_RoleId))
                            {

                                userInfo.ManageCompanyIds = userInfo.companyId;
                                userInfo.F_Level = 3;
                            }
                            else
                            {

    ...
    var page = {
        ...
        init: function ($page) {
            ...
            $header.find('.lr-form-header-more').on('tap', function () {
                learun.nav.go({ path: 'search', title: '', isBack: true, isHead: true, param: 'nworkflow/copytome' });// 告诉搜索页本身所在的地址
            });
            $page.find('#myflow_tab').toptab(['未读','已读'], function (_index) {
        ...
        loadData: function (param, callback, $page) {// 列表加载后台数据
            ...
            var url = '';
            var mypage = learun.nav.getpage('nworkflow/copytome');
            switch (mypage.currentPage) {
                case 0:
                    url = config.webapi + "learun/adms/newwf/mysendtask?type=0";
                    break;
                case 1:
                    url = config.webapi + "learun/adms/newwf/mysendtask?type=1";
                    break;
                //case 2:

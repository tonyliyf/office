...
namespace Learun.Application.WebApi
{
    ...
    public class BaseApi : NancyModule
    {
        ...
        private Response BeforeRequest(NancyContext ctx)
        {
            ...
            token = req.token;
            return null; //不做拦载，不做权限控制
            if (path == "/learun/adms/user/login" ||path == "/learun/adms/user/facelogin" || path == "/" || path == "/bgimg" || path == "/learun/adms/user/img" || path == "/learun/adms/desktop/img" || path == "/learun/adms/annexes/down"||path =="/learun/adms/apputil/info" || path == "/learun/adms/ProjectManager/Dc_EngineProject_info/pagelist")
            {// 登录接口，默认页面接口不做权限验证处理
                return null;
            }

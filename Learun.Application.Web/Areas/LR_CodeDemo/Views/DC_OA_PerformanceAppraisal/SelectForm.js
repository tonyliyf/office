...
var bootstrap = function ($, learun) {
    ...
    function renderUserlist(list) {
        ...
        for (var i = 0, l = list.length; i < l; i++) {
            ...
            //}
            if (userlistselected.indexOf(item.F_PostId) != -1) {
                active = "active";
            }
            var _cardbox = "";
    ...
    function addUser(useritem) {
        ...
        var _html = '<div class="user-selected-box" data-value="' + useritem.F_PostId + '" >';
        _html += '<p><span data-id="' + useritem.F_CompanyId + '"></span></p>';
        _html += '<p><span data-id="' + useritem.F_DepartmentId + '"></span>【' + useritem.F_Name + '】</p>';
        _html += '<span class="user-reomve" title="移除选中岗位"></span>';
        _html += '</div>';
        $warp.append(_html);
        learun.clientdata.getAsync('department', {
            ...
            $('#department_tree').lrtree({
                nodeClick: function (item) {
                    ...
                    if (!!userlist[item.id]) {
                        ...
                        learun.httpAsync('GET', top.$.rootUrl + '/LR_CodeDemo/DC_OA_PerformanceAppraisal/GetPostList', { companyId: companyId, departmentId: departmentId }, function (data) {
            ...
            $.lrSetForm(top.$.rootUrl + '/LR_CodeDemo/DC_OA_PerformanceAppraisal/GetPostIdList?tid=' + tid, function (data) {
                ...
                for (var i = 0, l = userList.length; i < l; i++) {
                    ...
                    if (!!item) {
                        ...
                        var _html = '<div class="user-selected-box" data-value="' + item.F_PostId + '" >';
                        _html += '<p><span data-id="' + item.F_CompanyId + '"></span></p>';
                        _html += '<p><span data-id="' + item.F_DepartmentId + '"></span>【' + item.F_Name + '】</p>';
                        _html += '<span class="user-reomve" title="移除选中岗位"></span>';
                        _html += '</div>';
                        $warp.append($(_html));
                        learun.clientdata.getAsync('department', {
                        ...
                        $('#user_list').find('[data-value="' + item.F_PostId + '"]').addClass('active');
                    }
                }
            });
        }
    ...
    acceptClick = function () {
        $.lrSaveForm(top.$.rootUrl + '/LR_CodeDemo/DC_OA_PerformanceAppraisal/InsertPostRalation', { tid: tid, postIds: String(userlistselected) }, function (res) { });
        return true;
    };
    page.init();
}
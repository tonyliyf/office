(function () {
    ...
    var getFormData = function ($page) {
        ...
        for (var id in formDataes) {
            if (!fieldMap[id]) {
                learun.layer.warning('未设置流程表单关联字段！', function () { }, '系统提示', '关闭');
                return false;
            }
    ...
    var page = {
        ...
        sign: function (info, auditers) {// info加签信息
            ...
            learun.httppost(config.webapi + "learun/adms/newwf/sign", flowreq, function (data) {
                ...
                var prepage = learun.nav.getpage(learun.storage.get('CurrentAuditPage'));
        ...
        learun.httpget(config.webapi + "learun/adms/newwf/processinfo", req, function (data) {
            ...
            if (data) {
                ...
                learun.custmerform.loadScheme(schemeIds, function (scheme) {
                    ...
                    var GetFieldName = function (_id) {
                        for (var field in scheme) {
                            if (scheme[field].data && scheme[field].data.length > 0) {
                                for (var i = 0; i < scheme[field].data.length; i++) {
                                    var componts = scheme[field].data[i].componts
                                    for (var j = 0; j < componts.length; j++) {
                                        if (componts[j].fieldsData && componts[j].fieldsData.length > 0) {
                                            for (var k = 0; k < componts[j].fieldsData.length; k++) {
                                                var fieldinfo = componts[j].fieldsData[k]
                                                if (_id.indexOf('|')>=0) {
                                                    _id = _id.split('|')[1]
                                                }
                                                if (fieldinfo.id == _id) {                                                    
                    ...
                    $.each(authorizeFields, function (_index, _item) {
                        if (_item.isLook === 0) {
                            var fieldname = GetFieldName(_item.fieldId)
                            if (fieldname && fieldname.length > 0) {
                                $('#nauditcontainer').find('#' + fieldname).parent().remove();
                            }
                        }
                        else if (_item.isEdit === 0) {
                            var fieldname = GetFieldName(_item.fieldId)
                            if (fieldname && fieldname.length > 0) {
                                $('#nauditcontainer').find('#' + fieldname).parent().attr('readonly', 'readonly');
                            }
                        }
                    });
                });

/*
 * 版 本 DC-FW V7.0.0 东雅图敏捷开发框架(http://www.dongyatu.com)
 * Copyright (c) 2013-2017 东雅图信息技术有限公司
 * 创建人：东雅图-前端开发组
 * 日 期：2018.7.12
 * 描 述：东雅图移动端框架(ADMS) 东雅图移动框架方法扩展
 */
(function ($, learun) {
    "use strict";
    ...
    learun.httpget = function (url, data, callback) {
        ...
        else { learun.layer.toast(url + '/[httpget]数据异常，请联系管理员！');}


        learun.http.get(url, param, function (res) {
            if (res === null) {
                learun.layer.toast(url+'/[httpget]无法连接服务器,请检测网络！');
                callback(null);
            }
                ...
                if (!learun.isOutLogin) {
                    ...
                    learun.layer.toast(url +'/[httpget]登录过期,请重新登录!');
    ...
    learun.httpcordovaget = function (url, data, callback) {
        var param = {};
        var logininfo = learun.storage.get('logininfo');
        if (!logininfo) {
            callback(null);
        ...
        learun.http.cordovaget(url, param, function (res) {
            if (res === null) {
                learun.layer.toast(url+'/[cordovaget]无法连接服务器,请检测网络！');
                callback(null);
            }
            else if (res.code === 410) {
                callback(null);
    ...
    learun.httppost = function (url, data, callback) {
        ...
        learun.http.post(url, param, function (res) {
            if (res === null) {
                learun.layer.toast(url+'/[httppost]无法连接服务器,请检测网络！');
                callback(null);
            }
            else if (res.code === 410) {
                callback(null);
                if (!learun.isOutLogin) {
                    learun.isOutLogin = true;
                    learun.layer.toast(url +'/[httppost]登录过期,请重新登录!');
                    learun.storage.set('logininfo', null);
                    learun.nav.go({ path: 'login', isBack: false, isHead: false });
                }
    ...
    clientAsyncData.company = {
        ...
        init: function () {
            if (clientAsyncData.company.states === loadSate.no) {
                ...
                var ver = data.ver || "";
                //learun.layer.loading(true, "公司信息正在加载中...");
    ...
    clientAsyncData.module = {
        ...
        init: function () {
            if (clientAsyncData.module.states === loadSate.no) {
                ...
                var ver = data.ver || "";
                learun.layer.loading(true, "菜单正在加载中...");
                //httpget或httpcordovaget
                learun.httpget(config.webapi + "learun/adms/function/list", { ver: ver }, function (data) {
                    learun.layer.loading(false);
                    //alert('菜单接口返回')
                    if (data) {
                        if (data.ver) {
                            //alert('更新菜单' + data.data.length)
    ...
    learun.myModule = {
        ...
        init: function (modules) {
            if (learun.myModule.states === loadSate.no) {
                ...
                var ver = mymoduleData.ver || "";
                learun.httpget(config.webapi + "learun/adms/function/mylist", { ver: ver }, function (data) {
                    if (data) {
                    ...
                    if (data) {
                        ...
                        $.each(data, function (_index, _item) {
                            if (_item.F_FileType === 'jpg' || _item.F_FileType === 'png'
                                || _item.F_FileType === 'docx' || _item.F_FileType === 'doc') {
                                var _point = {
                                    id: _item.F_Id,

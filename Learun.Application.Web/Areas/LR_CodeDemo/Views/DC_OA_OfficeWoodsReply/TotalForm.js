...
var bootstrap = function ($, learun) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#F_ReplyMonth').lrDataSourceSelect({
                code: 'GetMonth',
                value: 'itemvalue',
                text: 'text',
                select: function (data) {
                    if (data && parent.location.search.indexOf('type=create') >= 0) {
                        $.post(top.$.rootUrl + '/LR_CodeDemo/DC_OA_OfficeWoodsReply/GetDC_OA_OfficeWoodsReplyDetailTotalListByMonth', { month: data.text }, function (res) {
                            if (res.code == 200) {
                                $('#F_SumMoney').val(res.data.sum)
                                $('#DC_OA_OfficeWoodsReplyDetail').jfGridSet('refreshdata',res.data.table);
                            } else {

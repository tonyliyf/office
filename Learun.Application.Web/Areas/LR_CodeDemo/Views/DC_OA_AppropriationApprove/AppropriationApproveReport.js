    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#lr-print').on('click', function () {
                $("#gridtable").jqprintTable({ title: '资金拨付统计表' });
            });
            //导出
        ...
        search: function (param) {
            ...
            var explainField = function (columeName) {
                switch (columeName) {
                    ...
                    case 'f_approvemoney':
                        return '支出金额'
                    case 'f_appropriationcompany':
                        return '公司名称'
                    default:
                        return ''
                }
            }
            $.post(top.$.rootUrl + '/LR_CodeDemo/DC_OA_AppropriationApprove/GetAppropriationApproveData', param, function (res) {
                ...
                if (res.code == 200) {
                    ...
                    if (res.data.length > 0) {
                        for (var id in res.data[0]) {
                            ...
                            } else if (id == 'f_appropriationcompanyid') {
                                hdata.formatterAsync = function (callback, value, row, op, $cell) {
                            ...
                            } else if (id == 'f_createdepartmentid') {
                                hdata.formatterAsync = function (callback, value, row, op, $cell) {

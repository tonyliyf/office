...
(function (a, g) {
    Date.prototype.DateAdd = function (j, i) {
        ...
        switch (j) {
            case "s":
                return new Date(Date.parse(h) + (1000 * i));
            case "n":
                return new Date(Date.parse(h) + (60000 * i));
            case "h":
                return new Date(Date.parse(h) + (3600000 * i));
            case "d":
                return new Date(Date.parse(h) + (86400000 * i));
            case "w":
                return new Date(Date.parse(h) + ((86400000 * 7) * i));
            case "q":
                return new Date(h.getFullYear(), (h.getMonth()) + i * 3, h.getDate(), h.getHours(), h.getMinutes(), h.getSeconds());
            case "m":
                return new Date(h.getFullYear(), (h.getMonth()) + i, h.getDate(), h.getHours(), h.getMinutes(), h.getSeconds());
            case "y":
                return new Date((h.getFullYear() + i), h.getMonth(), h.getDate(), h.getHours(), h.getMinutes(), h.getSeconds())
        }
    };
    Date.prototype.DateDiff = function (j, h) {
        var i = this;
        if (typeof h == "string") {
            h = fui.date.parse(h)
        }
        switch (j) {
            case "s":
                return parseInt((h - i) / 1000);
            case "n":
                return parseInt((h - i) / 60000);
            case "h":
                return parseInt((h - i) / 3600000);
            case "d":
                return parseInt((h - i) / 86400000);
            case "w":
                return parseInt((h - i) / (86400000 * 7));
            case "m":
                return (h.getMonth() + 1) + ((h.getFullYear() - i.getFullYear()) * 12) - (i.getMonth() + 1);
            case "y":
    ...
    Date.prototype.MaxDayOfDate = function () {
        var k = this;
        var h = k.toArray();
        var i = (new Date(h[0], h[1] + 1, 1));
        var j = i.DateAdd("m", 1);
        var l = dateDiff(i.Format("yyyy-MM-dd"), j.Format("yyyy-MM-dd"));
        return l
    };
    ...
    var f = /matrix(3d)?\((.+?)\)/;
    var b = {};
    a.each(["Boolean", "Number", "String", "Function", "Array", "Date", "RegExp", "Object", "Error"],
        function (h, j) {
            b["[object " + j + "]"] = j.toLowerCase()
        });
    ...
    g.fui = {
        ...
        pageid: function () {
            ...
            var h = fui.nav.page.data[fui.nav.page.activeid] || {};
            return (h.op || {}).pageid || "no"
        },
        guid: function (l) {
            l = l || "_";
            var h = "";
        ...
        date: {
            ...
            parse: function (i) {
                ...
                if (typeof i === "string") {
                    if (i.indexOf("/Date(") > -1) {
                        h = new Date(parseInt(i.replace("/Date(", "").replace(")/", ""), 10))
                    } else {
                        h = new Date(Date.parse(i.replace(/-/g, "/").replace("T", " ").split(".")[0]))
                    }
                }
                return h
            },
            format: function (m, i) {
                ...
                if (/(y+)/.test(i)) {
                    i = i.replace(RegExp.$1, (h.getFullYear() + "").substr(4 - RegExp.$1.length))
                }
                for (var j in l) {
                    if (new RegExp("(" + j + ")").test(i)) {
                        i = i.replace(RegExp.$1, RegExp.$1.length == 1 ? l[j] : ("00" + l[j]).substr(("" + l[j]).length))
                    }
                }
                return i
            },
        ...
        createMask: function () {
            if (a(".f-backdrop").length == 0) {
                var h = '<div class="f-backdrop" ></div>';
                a("body").append(h);
                a(".f-backdrop").on("tap",
                    function () {
                        a(this).fadeOut();
                        a(".f-pop.active").removeClass("active");
                        return false
                    })
            }
        },
        ...
        hideMask: function () {
            a(".f-backdrop").fadeOut()
        },
        getStyles: function (h, i) {
            var j = h.ownerDocument.defaultView.getComputedStyle(h, null);
            if (i) {
                return j.getPropertyValue(i) || j[i]
            }
            return j
        },
        parseTranslateMatrix: function (l, j) {
            var i = l.match(f);
            var h = i && i[1];
            if (i) {
                i = i[2].split(",");
            ...
            if (j && k.hasOwnProperty(j)) {
                return k[j]
            }
            return k
        },
        type: function (h) {
            return h == null ? String(h) : b[{}.toString.call(h)] || "object"
        },
        isFunction: function (h) {
            return fui.type(h) === "function"
        },
        trigger: function (h, j, i) {
            if (h) {
                h.dispatchEvent(new CustomEvent(j, {
                    detail: i,
                    bubbles: true,
                    cancelable: true
                }))
        ...
        hooks: {},
        addAction: function (j, h) {
            var i = fui.hooks[j];
            if (!i) {
                i = []
            }
            h.index = h.index || 1000;
            i.push(h);
            i.sort(function (k, l) {
        ...
        doAction: function (i, h) {
            if (fui.isFunction(h)) {
                a.each(fui.hooks[i], h)
            } else {
                a.each(fui.hooks[i],
                    function (k, j) {
                        return !j.handle()
        ...
        offset: function (i) {
            ...
            return {
                top: h.top + g.pageYOffset - i.clientTop,
                left: h.left + g.pageXOffset - i.clientLeft
            }
        },
        includeJs: function (j) {
    ...
    a.extend = function (h) {
        ...
        d = false;
        for (var g in h) {
            i[g] = typeof h[g] == "function" && typeof e[g] == "function" && b.test(h[g]) ? (function (k, j) {
                return function () {
                    var m = this._super;
                    this._super = e[k];
    ...
    c.Class = a
})(window.fui); (function (b) {
    if (!b.requestAnimationFrame) {
        var a = 0;
        b.requestAnimationFrame = b.webkitRequestAnimationFrame ||
            function (c, e) {
                var d = new Date().getTime();
                var g = Math.max(0, 16.7 - (d - a));
        ...
        b.cancelAnimationFrame = b.webkitCancelAnimationFrame || b.webkitCancelRequestAnimationFrame ||
            function (c) {
                clearTimeout(c)
            }
    }
...
})(window.jQuery, window.fui); (function (l, C) {
    l.gestures = {
        session: {}
    };
    l.stopPropagation = function (D) {
        D.stopPropagation()
    };
    l.addGesture = function (D) {
        return l.addAction("gestures", D)
    };
    var x = Math.round;
    var a = Math.abs;
    var z = Math.sqrt;
    ...
    var r = function (F, D) {
        if (F.length >= 2 && D.length >= 2) {
            var E = ["pageX", "pageY"];
            return o(D[1], D[0], E) / o(F[1], F[0], E)
        }
        return 1
    };
    ...
    var q = function (F, D) {
        var E = ["pageX", "pageY"];
        return m(D[1], D[0], E) - m(F[1], F[0], E)
    };
    var t = function (D, E, F) {
        return {
            x: E / D || 0,
            y: F / D || 0
        }
    ...
    var k = function (D, E) {
        if (l.gestures.stoped) {
            return
        }
        l.doAction("gestures",
            function (G, F) {
                if (!l.gestures.stoped) {
                    if (l.options.gestureConfig[F.name] !== false) {
                        F.handle(D, E)
                    }
                }
            })
    ...
    var w = function () {
        return l.options.gestureConfig.pinch
    };
    var j = function (E) {
        var F = [];
        var D = 0;
        while (D < E.touches.length) {
            F[D] = {
                pageX: x(E.touches[D].pageX),
                pageY: x(E.touches[D].pageY)
            };
            D++
        }
        return {
    ...
    var f = function (I) {
        var H = l.gestures.session;
        var D = I.center;
        var E = H.offsetDelta || {};
        var F = H.prevDelta || {};
        var G = H.prevTouch || {};
        if (I.gesture.type === l.event.start || I.gesture.type === l.event.end) {
            F = H.prevDelta = {
                x: G.deltaX || 0,
                y: G.deltaY || 0
            };
        ...
        I.deltaX = F.x + (D.x - E.x);
        I.deltaY = F.y + (D.y - E.y)
    };
    var h = function (I) {
        var H = l.gestures.session;
        var J = I.touches;
        ...
        if (w() && K > 1 && !H.firstMultiTouch) {
            H.firstMultiTouch = j(I)
        } else {
            if (K === 1) {
                H.firstMultiTouch = false
            }
        }
        var F = H.firstTouch;
        var E = H.firstMultiTouch;
        var G = E ? E.center : F.center;
        var D = I.center = p(J);
        I.timestamp = l.now();
        I.deltaTime = I.timestamp - F.timestamp;
        I.angle = m(G, D);
        I.distance = o(G, D);
        f(I);
        I.offsetDirection = n(I.deltaX, I.deltaY);
        I.scale = E ? r(E.touches, J) : 1;
        I.rotation = E ? q(E.touches, J) : 0;
        g(I)
    };
    var e = 25;
    var g = function (J) {
        var I = l.gestures.session;
        var H = I.lastInterval || J;
        var D = J.timestamp - H.timestamp;
        var L;
        var M;
        var N;
        var G;
        if (J.gesture.type != l.event.cancel && (D > e || H.velocity === undefined)) {
            var E = H.deltaX - J.deltaX;
            var F = H.deltaY - J.deltaY;
            var K = t(D, E, F);
            M = K.x;
            N = K.y;
            L = (a(K.x) > a(K.y)) ? K.x : K.y;
            G = n(E, F) || H.direction;
            I.lastInterval = J
        } else {
            L = H.velocity;
    ...
    var s = function (H, L) {
        var D = i(l.slice.call(H.touches || [H]));
        var N = H.type;
        var K = [];
        var E = [];
        if ((N === l.event.start || N === l.event.move) && D.length === 1) {
            A[D[0].identifier] = true;
            K = D;
            E = D;
            L.target = H.target
        } else {
            ...
            var K = [];
            var E = [];
            var F = i(l.slice.call(H.changedTouches || [H]));
            L.target = H.target;
            var J = l.gestures.session.target || H.target;
            K = D.filter(function (O) {
                return v(O.target, J)
            });
            if (N === l.event.start) {
                I = 0;
                while (I < K.length) {
                    A[K[I].identifier] = true;
                    I++
                }
            }
            I = 0;
            while (I < F.length) {
                ...
                if (N === l.event.end || N === l.event.cancel) {
                    delete A[F[I].identifier]
                }
                I++
            }
            if (!E.length) {
        ...
        K = B(K.concat(E), "identifier", true);
        var M = K.length;
        var G = E.length;
        if (N === l.event.start && M - G === 0) {
            L.isFirst = true;
            l.gestures.touch = l.gestures.session = {
                target: H.target
            }
        }
        L.isFinal = ((N === l.event.end || N === l.event.cancel) && (M - G === 0));
        L.touches = K;
        L.changedTouches = E;
        return true
    };
    var u = function (D) {
        ...
        h(E);
        k(D, E);
        l.gestures.session.prevTouch = E;
        if (D.type === l.event.end && !l.event.supportTouch) {
            l.gestures.touch = l.gestures.session = {}
        }
        D = D || C.event;
        l.preventDefault(D);
    ...
    var d = $("body")[0];
    d.addEventListener(l.event.start, u);
    d.addEventListener(l.event.move, u, {
        passive: false
    });
    d.addEventListener(l.event.end, u);
    d.addEventListener(l.event.cancel, u);
    l.isScrolling = false;
    var y = null;
...
})(window.fui, window); (function (a, c) {
    var b = function (d, f) {
        ...
        switch (d.type) {
            case a.event.start:
                break;
            case a.event.move:
                if (!f.direction || !e.target) {
                    return
                }
                if (e.lockDirection && e.startDirection) {
                    if (e.startDirection && e.startDirection !== f.direction) {
                        if (e.startDirection === "up" || e.startDirection === "down") {
                            f.direction = f.deltaY < 0 ? "up" : "down"
                        } else {
                            f.direction = f.deltaX < 0 ? "left" : "right"
                        }
                    }
                }
                if (!e.drag) {
                    e.drag = true;
                    a.trigger(e.target, c + "start", f)
                }
                a.trigger(e.target, c, f);
                a.trigger(e.target, c + f.direction, f);
                break;
            case a.event.end:
            case a.event.cancel:
                if (e.drag && f.isFinal) {
                    a.trigger(e.target, c + "end", f)
                }
                break
        }
    };
...
})(fui, "drag"); (function (b, d) {
    var a = 0;
    var c = function (e, i) {
        var h = b.gestures.session;
        var g = this.options;
        var f = b.now();
        switch (e.type) {
            case b.event.move:
                if (f - a > 300) {
                    a = f;
                    h.flickStart = i.center
                }
                break;
            case b.event.end:
            case b.event.cancel:
                i.flick = false;
                if (h.flickStart && g.flickMaxTime > (f - a) && i.distance > g.flickMinDistince) {
                    i.flick = true;
                    i.flickTime = f - a;
                    i.flickDistanceX = i.center.x - h.flickStart.x;
                    i.flickDistanceY = i.center.y - h.flickStart.y;
                    b.trigger(h.target, d, i);
                    b.trigger(h.target, d + i.direction, i)
                }
                break
        }
    };
    ...
    var b = function (e, h) {
        ...
        switch (e.type) {
            case a.event.start:
                if (a.options.gestureConfig.hold) {
                    d && clearTimeout(d);
                    d = setTimeout(function () {
                        h.hold = true;
                        a.trigger(g.target, c, h)
                    },
                        f.holdTimeout)
                }
                break;
...
})(fui, "hold"); (function (a, c) {
    var b = function (d, k) {
        var f = this.options;
        var j = a.gestures.session;
        switch (d.type) {
            case a.event.start:
                break;
            case a.event.move:
                if (a.options.gestureConfig.pinch) {
                    ...
                    if (!j.pinch) {
                        j.pinch = true;
                        a.trigger(j.target, c + "start", k)
                    }
                    a.trigger(j.target, c, k);
                    var h = k.scale;
                    var g = k.rotation;
                    var e = typeof k.lastScale === "undefined" ? 1 : k.lastScale;
                    var i = 1e-12;
                    if (h > e) {
                        e = h - i;
                        a.trigger(j.target, c + "out", k)
                    } else {
                        if (h < e) {
                            e = h + i;
                            a.trigger(j.target, c + "in", k)
                        }
                    }
                    if (Math.abs(g) > f.minRotationAngle) {
                        a.trigger(j.target, "rotate", k)
                    }
                }
                break;
            case a.event.end:
            case a.event.cancel:
                if (a.options.gestureConfig.pinch && j.pinch && k.touches.length === 2) {
                    j.pinch = false;
                    a.trigger(j.target, c + "end", k)
                }
                break
        }
    };
...
})(fui, "pinch"); (function (a, c) {
    var b = function (d, g) {
        var f = a.gestures.session;
        if (d.type === a.event.end || d.type === a.event.cancel) {
            var e = this.options;
            g.swipe = false;
            if (g.direction && e.swipeMaxTime > g.deltaTime && g.distance > e.swipeMinDistince) {
                g.swipe = true;
                a.trigger(f.target, c, g);
                a.trigger(f.target, c + g.direction, g)
            }
        }
    };
    a.addGesture({
    ...
    var b = {
        ...
        open: function (k, j) {
            var l = null;
            if (d.nav.page.data[k.id]) {
                l = d.nav.page.data[k.id];
                l.load = false;
                var h = l.op.fclass;
                l.preid = d.nav.page.activeid || "";
                l.op = k;
                l.$page.removeAttr("style");
                l.$page.attr("data-type", k.type);
                l.$page.removeClass(h);
                l.$page.addClass(k.fclass);
                l.hasLoad = true
            } else {
                k.pageid = d.guid();
                var i = '<div class="f-page ' + k.fclass + '" data-type="' + k.type + '" id="' + k.pageid + '">';
                i += '<div class="f-page-header" >';
                i += '<div class="f-page-backbtn" > ' + k.backbtn + " </div>";
                i += '<div class="f-page-title">' + k.title + "</div>";
                i += "</div>";
                i += '<div class="f-page-body"></div>';
                i += "</div>";
                a("body").append(i);
                l = {
                    $page: a("#" + k.pageid),
                    preid: d.nav.page.activeid || "",
                    op: k,
                    load: false
                };
                d.nav.page.data[k.id] = l;
                d.nav.page.count++;
                b.bindEvent(l.$page)
            }
            d.nav.page.activeid = k.id;
            k.start && k.start(l);
            switch (k.type) {
                case "right":
                    l.$page.animate({
        ...
        close: function (k, i) {
            var j = k.op;
            if (j.bfdestroy) {
                if (!j.bfdestroy(k, "backbtn")) {
                    i()
                }
            }
            d.nav.page.activeid = "";
            if (k.preid) {
                var h = d.nav.page.data[k.preid];
                h.load = false;
                h.$page.show();
                d.nav.page.activeid = k.preid;
                h = null
            }
            k.$page.css({
                "z-index": 3
        ...
        bindEvent: function (h) {
            h.find(".f-page-backbtn").on("tap",
                function () {
                    var i = d.nav.page.activeid;
                    var j = d.nav.page.data[i];
                    if (j.preid == 'nworkflow/copytome') {
                        if (j.op && j.op.param && j.op.param.taskId) {
                            window.lrmui.httpget(config.webapi + "learun/adms/newwf/mytaskstate?taskid=" + j.op.param.taskId, { queryJson: {} }, function () { })
                        }
                    }
                    if (!j.load) {
                        return false
        ...
        removePage: function (i) {
            i.$page.remove();
            i.$page = null;
            a('[data-page="' + i.op.pageid + '"]').remove();
            i.op.destroy && i.op.destroy(i);
            if (i.preid) {
                var h = d.nav.page.data[i.preid];
                if (h.op.end) {
                    h.hasLoad = true;
            ...
            d.nav.page.count--;
            d.nav.page.data[i.op.id] = null
        },
        remove: function (h) {
            var i = d.nav.page.data[h];
            if (i != null) {
                i.$page.remove();
                i.$page = null;
                a('[data-page="' + i.op.pageid + '"]').remove();
...
})(window.jQuery, window.fui); (function (a, b) {
    b.layer = {
        open: function (f) {
            var e = {
                title: "",
            ...
            a.extend(e, f || {});
            e.layerid = b.guid();
            var d = '            <div class="f-layer" id="' + e.layerid + '">                <div class="f-layer-closebtn" >关闭</div>                <div class="f-layer-cancelbtn" >取消</div>                <div class="f-layer-page" data-level="0" >                    <div class="f-layer-page-header" >                        <div class="f-layer-page-title">' + e.title + '</div>                    </div>                    <div class="f-layer-page-body"></div>                </div>                <div class="f-layer-mask" ></div>            </div>';
            a("body").append(d);
            var c = a("#" + e.layerid);
            c[0].fop = e;
            c.find(".f-layer-cancelbtn").on("tap",
                function () {
                    var h = a(this);
                    var g = h.parent();
                    g.animate({
            ...
            c.find(".f-layer-closebtn").on("tap",
                function () {
                    var h = a(this);
                    var g = h.parent();
                    var j = g[0].fop;
                    var i = g.find('[data-level="' + j.level + '"]');
                    g.find(".f-layer-closebtn").hide();
                    g.find(".f-layer-page").show();
                    i.animate({
                        left: "100%"
                    },
                        200,
                        function () {
                            var k = a(this);
                            var l = k.parent()[0].fop;
                            l.level = 0;
                            setTimeout(function () {
                                g.find(".f-layer-left-page").remove();
                                l.close && l.close(l.layerid, l.level, l);
                                g = null
                            },
                                100)
                        })
            ...
            c.animate({
                ...
                function () {
                    var g = a(this)[0].fop;
                    setTimeout(function () {
                        var h = a("#" + g.layerid + ' [data-level="' + g.level + '"] .f-layer-page-body');
                        g.open && g.open(h, g.layerid, g.level, g)
                    },
                        100)
                });
            c = null
        ...
        iopen: function (h, i) {
            var c = a("#" + h);
            var d = c.find(".f-layer-mask");
            d.show();
            var g = c[0].fop;
            g.level = g.level + 1;
            var f = '            <div class="f-layer-left-page" data-level="' + g.level + '" >                <div class="f-layer-page-header" >                    <div class="f-layer-page-backbtn" >' + g.backbtn + '</div>                    <div class="f-layer-page-title">' + i + '</div>                </div>                <div class="f-layer-page-body"></div>            </div>';
            c.append(f);
            var e = c.find('[data-level="' + g.level + '"]');
            e.find(".f-layer-page-backbtn").on("tap",
                function () {
                    var j = a(this);
                    var l = j.parents(".f-layer-left-page");
                    var k = l.parent();
                    var m = k[0].fop;
                    m.level = m.level - 1;
                    if (m.level < 2) {
                    ...
                    k.find('[data-level="' + m.level + '"]').show();
                    l.css({
                        "z-index": 5
                    });
                    l.animate({
                        ...
                        function () {
                            var n = a(this);
                            var o = n.parent()[0].fop;
                            setTimeout(function () {
                                n.remove();
                                o.close && o.close(o.layerid, o.level, o)
                            },
                                100)
                        })
                });
            e.animate({
                ...
                function () {
                    ...
                    var l = j[0].fop;
                    if (l.level >= 2) {
                        j.find(".f-layer-closebtn").show()
                    }
                    j.find('[data-level="' + (l.level - 1) + '"]').hide();
                    setTimeout(function () {
                        var m = k.find(".f-layer-page-body");
                        l.open && l.open(m, l.layerid, l.level, l)
                    },
                        100);
                    d.hide()
                });
        ...
        close: function (c) {
            a("#" + c + " .f-layer-cancelbtn").trigger("click")
        },
        iclose: function (c) {
            a("#" + c + " .f-layer-closebtn").trigger("click")
        },
        closeLayer: function (d) {
            var c = a("#" + d);
            var e = c[0].fop.level;
            c.find('[data-level="' + e + '"] .f-layer-page-backbtn').trigger("click")
        }
    }
})(window.jQuery, window.fui); (function (a, c) {
    var e = navigator.platform.toLowerCase();
    var f = navigator.userAgent.toLowerCase();
    var d = (f.indexOf("iphone") > -1 || f.indexOf("ipad") > -1 || f.indexOf("ipod") > -1) && (e.indexOf("iphone") > -1 || e.indexOf("ipad") > -1 || e.indexOf("ipod") > -1);
    var b = {
        init: function (g, i) {
            var h = '<div class="f-pciker-rule"></div><ul class="f-pciker-list"></ul>';
            g.addClass("f-picker");
            g.html(h);
            b.initParams(g);
            b.initData(g, i);
        ...
        initParams: function (g) {
            var h = g[0];
            h.list = g.find("ul")[0];
            h.height = h.offsetHeight;
            h.r = h.height / 2 - 10;
            h.d = h.r * 2;
            h.itemHeight = 40;
            h.itemAngle = parseInt(b.calcAngle(h, h.itemHeight * 0.8));
            h.hightlightRange = h.itemAngle / 2;
            h.visibleRange = 90;
            h.beginAngle = 0;
            h.beginExceed = h.beginAngle - 30;
            h.list.angle = h.beginAngle;
            h.lastMoveTime = 0;
            h.lastMoveStart = 0;
            h.stopInertiaMove = false;
            h.lastAngle = 0;
            h.startY = null;
            h.isPicking = false;
            h.selected = null;
            h.preselected = null;
            h.list.style.webkitTransition = "150ms ease-out";
            if (d) {
                h.list.style.webkitTransformOrigin = "center center " + h.r + "px"
            }
        },
        initData: function (i, k) {
            var g = i.find("ul");
            g.html("");
            var l = i[0];
            l.items = [];
            ...
            a.each(k.data,
                function (m, o) {
                    ...
                    l.items.push(n);
                    if (m <= 20) {
                        j += '<li data-index="' + m + '" style="transform-origin: center center -' + l.r + "px; transform: translateZ(" + l.r + "px) rotateX(-" + n.angle + 'deg);">' + n.text + "</li>"
                    }
                });
            l.beginindex = 0;
            l.endindex = 20;
            l.endExceed = l.endAngle + 30;
            g.html(j);
            b.setAngle(l, l.beginAngle);
            if (!!l.selected) {
                var h = a(l);
                h.trigger("change");
                if (!!k.change) {
                    k.change(l.selected.obj, h)
                }
            }
        },
        bindEvent: function (g) {
            var h = g[0];
            h.addEventListener(c.event.start,
                function (i) {
                    var j = a(this)[0];
                    j.isPicking = true;
                    c.preventDefault(i);
                    j.list.style.webkitTransition = "";
                    j.startY = (i.changedTouches ? i.changedTouches[0] : i).pageY;
                    j.lastAngle = j.list.angle;
                    b.updateInertiaParams(j, i, true)
                },
                false);
            h.addEventListener(c.event.end,
                function (i) {
                    var j = a(this)[0];
                    j.isPicking = false;
                    c.preventDefault(i);
                    b.startInertiaScroll(j, i)
                },
                false);
            h.addEventListener(c.event.cancel,
                function (i) {
                    var j = a(this)[0];
                    j.isPicking = false;
                    c.preventDefault(i);
                ...
                false);
            h.addEventListener(c.event.move,
                function (l) {
                    var n = a(this)[0];
                    if (!n.isPicking) {
                        return
                    ...
                    c.preventDefault(l);
                    var k = (l.changedTouches ? l.changedTouches[0] : l).pageY;
                    var j = k - n.startY;
                    var i = b.calcAngle(n, j);
                    var m = j > 0 ? n.lastAngle - i : n.lastAngle + i;
                    if (m > n.endExceed) {
                        m = n.endExceed
                    }
                    if (m < n.beginExceed) {
                    ...
                    b.setAngle(n, m);
                    b.updateInertiaParams(n, l)
                },
                false)
        },
        updateInertiaParams: function (k, g, h) {
            var j = g.changedTouches ? g.changedTouches[0] : g;
            if (h) {
                k.lastMoveStart = j.pageY;
                k.lastMoveTime = g.timeStamp || Date.now();
                k.startAngle = k.list.angle
            } else {
                var i = g.timeStamp || Date.now();
                if (i - k.lastMoveTime > 300) {
                    k.lastMoveTime = i;
                    k.lastMoveStart = j.pageY
                }
            }
            k.stopInertiaMove = true
        },
        startInertiaScroll: function (o, l) {
            var n = l.changedTouches ? l.changedTouches[0] : l;
            var m = l.timeStamp || Date.now();
            var r = (n.pageY - o.lastMoveStart) / (m - o.lastMoveTime);
            var h = r > 0 ? -1 : 1;
            var g = h * 0.0006 * -1;
            var k = Math.abs(r / g);
            var i = r * k / 2;
            var q = o.list.angle;
            var j = b.calcAngle(o, i) * h;
            var p = j;
            if (q + j < o.beginExceed) {
                j = o.beginExceed - q;
                k = k * (j / p) * 0.6
            }
            if (q + j > o.endExceed) {
            ...
            b.scrollDistAngle(o, m, q, j, k)
        },
        scrollDistAngle: function (j, i, k, g, h) {
            j.stopInertiaMove = false; (function (p, q, l, m) {
                var n = 13;
                var r = m / n;
                var s = 0; (function o() {
                    if (j.stopInertiaMove) {
                        return
                    }
                    var t = b.quartEaseOut(s, q, l, r);
                    b.setAngle(j, t);
                    s++;
                    if (s > r - 1 || t < j.beginExceed || t > j.endExceed) {
                        b.endScroll(j);
                        return
                    }
                    setTimeout(o, n)
        ...
        endScroll: function (j) {
            if (j.list.angle < j.beginAngle) {
                j.list.style.webkitTransition = "150ms ease-out";
                b.setAngle(j, j.beginAngle)
            } else {
                if (j.list.angle > j.endAngle) {
                    j.list.style.webkitTransition = "150ms ease-out";
                    b.setAngle(j, j.endAngle)
                } else {
                    var h = parseInt((j.list.angle / j.itemAngle).toFixed(0));
                    j.list.style.webkitTransition = "100ms ease-out";
                    b.setAngle(j, j.itemAngle * h)
                }
            }
            if (j.preselected != j.selected) {
                var g = a(j);
                var i = j.fop;
                g.trigger("change");
                if (!!i.change) {
                    i.change(j.selected.obj, g)
                }
            }
        },
        calcElementItemVisibility: function (o, l) {
            var h = a(o);
            var g = h.find("ul");
            var k = "";
            var q = 0;
            var p = o.items.length;
            a.each(o.items,
                function (i, s) {
                    var r = Math.abs(s.angle - l);
                    if (r < o.hightlightRange) {
                        o.selected = s;
                        if (!s.hightlight || !g.find('li[data-index="' + i + '"]').hasClass("highlight")) {
                            s.hightlight = true;
                            g.find('li[data-index="' + i + '"]').addClass("highlight")
                        }
                    } else {
                        if (r < o.visibleRange) {
                            if (!s.visible || !g.find('li[data-index="' + i + '"]').hasClass("visible")) {
                                s.visible = true;
                                g.find('li[data-index="' + i + '"]').addClass("visible")
                            }
                            if (!!s.hightlight || g.find('li[data-index="' + i + '"]').hasClass("highlight")) {
                                s.hightlight = false;
                                g.find('li[data-index="' + i + '"]').removeClass("highlight")
                            }
                            if (p > i) {
                                p = i
                            }
                            ...
                            if (!!s.visible || g.find('li[data-index="' + i + '"]').hasClass("visible")) {
                                s.visible = false;
                                g.find('li[data-index="' + i + '"]').removeClass("visible")
                            }
                            if (!!s.hightlight || g.find('li[data-index="' + i + '"]').hasClass("highlight")) {
                                s.hightlight = false;
                                g.find('li[data-index="' + i + '"]').removeClass("highlight")
            ...
            if (o.items.length > 21) {
                if (q >= o.endindex) {
                    var n = o.endindex + 11;
                    for (var m = o.endindex + 1; m < n; m++) {
                        if (!!o.items[m]) {
                            var j = "";
                            if (!!o.items[m].visible) {
                                j = 'class="visible"'
                            }
                            g.append("<li " + j + ' data-index="' + m + '"  style="transform-origin: center center -' + o.r + "px; transform: translateZ(" + o.r + "px) rotateX(-" + o.items[m].angle + 'deg);">' + o.items[m].text + "</li>");
                            g.find('li[data-index="' + (m - 21) + '"]').remove();
                            o.endindex++;
                            o.beginindex++
                        }
                    }
                    ...
                    if (p <= o.beginindex) {
                        var n = o.beginindex - 11;
                        for (var m = o.beginindex - 1; m > n; m--) {
                            if (!!o.items[m]) {
                                var j = "";
                                if (!!o.items[m].visible) {
                                    j = 'class="visible"'
                                }
                                g.prepend("<li " + j + ' data-index="' + m + '"  style="transform-origin: center center -' + o.r + "px; transform: translateZ(" + o.r + "px) rotateX(-" + o.items[m].angle + 'deg);">' + o.items[m].text + "</li>");
                                g.find('li[data-index="' + (m + 21) + '"]').remove();
                                o.endindex--;
                                o.beginindex--
                            }
        ...
        setAngle: function (h, g) {
            h.list.angle = g;
            h.list.style.webkitTransform = "perspective(1000px) rotateY(0deg) rotateX(" + g + "deg)";
            b.calcElementItemVisibility(h, g)
        },
        calcAngle: function (m, j) {
            var g = parseFloat(m.r);
            var i = parseFloat(m.r);
            j = Math.abs(j);
            var l = parseInt(j / m.d) * 180;
            j = j % m.d;
            var k = (g * g + i * i - j * j) / (2 * g * i);
            var h = l + b.rad2deg(Math.acos(k));
            return h
        },
        rad2deg: function (g) {
            return g / (Math.PI / 180)
        },
        quartEaseOut: function (j, g, h, i) {
            return - h * ((j = j / i - 1) * j * j * j - 1) + g
        }
    };
    a.fn.fpicker = function (i) {
        var h = {
            data: [],
            value: "value",
            text: "text",
    ...
    a.fn.fpickerSetData = function (h) {
        var g = a(this);
        if (!g[0].fop) {
            return g
        }
        g[0].fop.data = h;
        b.initParams(g);
        b.initData(g, g[0].fop);
        return g
    };
    a.fn.fpickerSet = function (i) {
        var g = a(this);
        var h = g[0];
        if (!h.fop) {
            return g
        ...
        a.each(h.items,
            function (l, n) {
                if (n.value == i) {
                    b.setAngle(h, n.angle);
                    if (h.items.length > 21) {
                        var j = g.find("ul");
                        j.html("");
                        h.beginindex = n.id - 10;
                        h.endindex = n.id + 10;
                        for (var m = h.beginindex; m < h.endindex + 1; m++) {
                            if (!!h.items[m]) {
                                var k = "";
                                if (!!h.items[m].visible) {
                                    k = 'class="visible"'
                                }
                                if (!!h.items[m].hightlight) {
                                    k = 'class="highlight"'
                                }
                                j.append("<li " + k + ' data-index="' + m + '"  style="transform-origin: center center -' + h.r + "px; transform: translateZ(" + h.r + "px) rotateX(-" + h.items[m].angle + 'deg);">' + h.items[m].text + "</li>")
                            }
                        }
                        if (!!h.selected) {
                            g.trigger("change");
                            if (!!h.fop.change) {
                                h.fop.change(h.selected.obj, g)
                            }
                        }
                    }
                    return false
    ...
    a.fn.fpickerGet = function () {
        var g = a(this);
        return g[0].selected.value || ""
    };
    a.fn.fpickerGetObj = function () {
        var g = a(this);
        return g[0].selected
    }
})(window.jQuery, window.fui); (function (a, c) {
    var b = {
        init: function (d, f) {
            f.id = c.guid();
            var e = '<div class="f-poppicker f-pop" id="pop_' + f.id + '"  data-page="' + c.pageid() + '"  >';
            e += '<div class="f-poppicker-header" >';
            e += '<div class="f-poppicker-cancel">取消</div>';
            e += '<div class="f-poppicker-ok">确定</div>';
            e += "</div>";
            e += '<div class="f-poppicker-body" ></div>';
            e += "</div>";
            a("body").append(e);
            c.createMask();
            b.initPicker(f);
        ...
        bindEvent: function (e, f) {
            ...
            var d = a("#pop_" + f.id);
            d.find(".f-poppicker-cancel").on("tap",
                function () {
                    var g = a(this).parents(".f-poppicker");
                    g.removeClass("active");
                    c.hideMask();
                    return false
                });
            d.find(".f-poppicker-ok").on("tap", {
                $self: e,
                fop: f
            },
                function (l) {
                    var h = a(this).parents(".f-poppicker");
                    ...
                    var q = [];
                    var k = [];
                    for (var n = 0; n < m.level; n++) {
                        var g = a("#picker_" + m.id + "_" + n);
                        var o = g.fpickerGetObj();
                        p.push(o.text);
                        q.push(o.value);
                        k.push(o)
                    }
                    if (m.value != String(q)) {
                        ...
                        j.trigger("change");
                        if (!!m.change) {
                            m.change(m.value, m.text, k, j)
                        }
                    }
                    return false
                })
        },
        initPicker: function (g) {
            var e = a("#pop_" + g.id + " .f-poppicker-body");
            for (var f = 0; f < g.level; f++) {
                var d = a('<div id="picker_' + g.id + "_" + f + '"  data-level="' + f + '"  ></div>');
                d.css("width", (100 / g.level + "%"));
                e.append(d);
                d.fpicker({
                    data: [],
                    value: g.ivalue,
                    text: g.itext,
                    change: function (k, h) {
                        var j = parseInt(h.attr("data-level")) + 1;
                        if (g.level > 1 && j < g.level) {
                            var i = a("#picker_" + g.id + "_" + j);
                            i.fpickerSetData(k.children)
                        }
                    }
                })
            }
            a("#picker_" + g.id + "_0").fpickerSetData(g.data)
        }
    };
    a.fn.fpoppicker = function (f) {
        var e = {
            data: [],
            level: 1,
            change: false,
    ...
    a.fn.fpoppickerSet = function (h) {
        ...
        if (h != undefined && h != "" && h != null) {
            ...
            a.each(i,
                function (k, m) {
                    var j = a("#picker_" + f.id + "_" + k);
                    j.fpickerSet(m);
                    var l = j.fpickerGetObj();
                    if (l) {
                        g.push(l.text || "");
                        e.push(l)
                    }
            ...
            d.trigger("change");
            if (!!f.change) {
                f.change(f.value, f.text, e, d)
            }
        }
    };
    a.fn.fpoppickerSetData = function (e) {
        var d = a(this);
        if (d.length > 0) {
            var f = d[0].fop;
            if (f) {
                a("#picker_" + f.id + "_0").fpickerSetData(e)
            }
        }
        d = null
    }
    ...
    var b = {
        init: function (e, h) {
            h.id = c.guid();
            var g = '<div class="f-dtpicker f-pop" id="dt_' + h.id + '" data-type="' + h.type + '"   data-page="' + c.pageid() + '"  >';
            g += '<div class="f-dtpicker-header" >';
            g += '<div class="f-dtpicker-cancel">取消</div>';
            g += '<div class="f-dtpicker-ok">确定</div>';
            g += "</div>";
            g += '<div class="f-dtpicker-title">';
            g += '<h5 data-id="title-y">' + h.label[0] + "</h5>";
            g += '<h5 data-id="title-m">' + h.label[1] + "</h5>";
            g += '<h5 data-id="title-d">' + h.label[2] + "</h5>";
            g += '<h5 data-id="title-h">' + h.label[3] + "</h5>";
            g += '<h5 data-id="title-i">' + h.label[4] + "</h5>";
            g += "</div>";
            g += '<div class="f-dtpicker-body" ></div>';
            g += "</div>";
            a("body").append(g);
            c.createMask();
            switch (h.type) {
        ...
        bindEvent: function (e, f) {
            ...
            var d = a("#dt_" + f.id);
            d.find(".f-dtpicker-cancel").on("tap",
                function () {
                    var g = a(this).parents(".f-dtpicker");
                    g.removeClass("active");
                    c.hideMask();
                    return false
                });
            d.find(".f-dtpicker-ok").on("tap", {
                $self: e,
                fop: f
            },
                function (i) {
                    ...
                    switch (j.type) {
                        case "datetime":
                            var k = b.getfYear(j) + "-" + b.getfMonth(j) + "-" + b.getfDay(j) + " " + b.getfHour(j) + ":" + b.getfMinute(j);
                            l = c.date.format(k, j.format || "yyyy-MM-dd hh:mm");
                            break;
                        case "date":
                            var k = b.getfYear(j) + "-" + b.getfMonth(j) + "-" + b.getfDay(j);
                            l = c.date.format(k, j.format || "yyyy-MM-dd");
                            break;
                        case "time":
                            var k = "2017-12-18 " + b.getfHour(j) + ":" + b.getfMinute(j);
                            l = c.date.format(k, j.format || "hh:mm");
                            break;
                        case "month":
                            var k = b.getfYear(j) + "-" + b.getfMonth(j) + "-01";
                            l = c.date.format(k, j.format || "yyyy-MM");
                            break
                    }
                    if (j.value != l) {
                        j.value = l;
        ...
        initYear: function (l) {
            ...
            var e = a("#dt_" + l.id + " .f-dtpicker-body");
            var d = a('<div id="picker_' + l.id + '_y" class="f-dt-picker f-dt-y" ></div>');
            e.append(d);
            d.fpicker({
                data: h
            }).fpickerSet(m);
        ...
        initMonth: function (h) {
            var f = new Date();
            var g = f.getMonth() + 1;
            var e = a("#dt_" + h.id + " .f-dtpicker-body");
            var d = a('<div id="picker_' + h.id + '_m" class="f-dt-picker f-dt-m" ></div>');
            e.append(d);
            d.fpicker({
                data: [{
                    text: "01",
        ...
        initDay: function (k, g, d) {
            var h = new Date();
            var i = h.getDate();
            var j = d.fpickerGet();
            var l = g.fpickerGet();
            var f = a("#dt_" + k.id + " .f-dtpicker-body");
            var e = a('<div id="picker_' + k.id + '_d" class="f-dt-picker f-dt-d" ></div>');
            f.append(e);
            e.fpicker({
                data: b.getDayData(l, j)
            }).fpickerSet(i);
            g[0].fop.change = function (t, m) {
                var r = t.value;
                var q = d.fpickerGet();
                var o = e.fpickerGet();
                var p = e[0].fop.data.length;
                var s = b.getDayNum(r, q);
                if (p != s) {
                    if (o > s) {
                        o = s
                    }
                    var n = b.getDayData(r, q);
                    e.fpickerSetData(n).fpickerSet(o)
                }
            };
            d[0].fop.change = function (t, m) {
                var q = t.value;
                var r = g.fpickerGet();
                var o = e.fpickerGet();
                var p = e[0].fop.data.length;
        ...
        initHour: function (i) {
            ...
            var f = new Date();
            var h = f.getHours();
            var e = a("#dt_" + i.id + " .f-dtpicker-body");
            var d = a('<div id="picker_' + i.id + '_h" class="f-dt-picker f-dt-h" ></div>');
            e.append(d);
            d.fpicker({
                data: g
            }).fpickerSet(h)
        },
        initMinute: function (i) {
            var g = [{
                text: "00",
                value: 0
            },
            ...
            var h = f.getMinutes();
            var e = a("#dt_" + i.id + " .f-dtpicker-body");
            var d = a('<div id="picker_' + i.id + '_i" class="f-dt-picker f-dt-i" ></div>');
            e.append(d);
            d.fpicker({
                data: g
            }).fpickerSet(h)
        },
        getDayData: function (j, h) {
            var f = [];
            for (var g = 1; g <= b.getDayNum(j, h); g++) {
                var e = "";
                if (g < 10) {
                    e = "0" + g
                } else {
        ...
        getDayNum: function (e, d) {
            if ([1, 3, 5, 7, 8, 10, 12].indexOf(d) > -1) {
                return 31
            } else {
                if ([4, 6, 9, 11].indexOf(d) > -1) {
                    return 30
                } else {
                    if (c.date.isLeapYear(e)) {
                        return 29
                    } else {
                        return 28
                    }
        ...
        getfYear: function (e) {
            var d = a("#picker_" + e.id + "_y");
            return d.fpickerGet()
        },
        getfMonth: function (e) {
            var d = a("#picker_" + e.id + "_m");
            return d.fpickerGetObj().text
        },
        getfDay: function (e) {
            var d = a("#picker_" + e.id + "_d");
            return d.fpickerGetObj().text
        },
        getfHour: function (e) {
            var d = a("#picker_" + e.id + "_h");
            return d.fpickerGetObj().text
        },
        getfMinute: function (e) {
            var d = a("#picker_" + e.id + "_i");
            return d.fpickerGetObj().text
        },
        setValue: function (k, l) {
            ...
            var j = c.date.parse(l);
            var i = a("#picker_" + k.id + "_y");
            if (i.length > 0) {
                i.fpickerSet(j.getFullYear())
            }
            var h = a("#picker_" + k.id + "_m");
            if (h.length > 0) {
                h.fpickerSet(j.getMonth() + 1)
            }
            var e = a("#picker_" + k.id + "_d");
            if (e.length > 0) {
                e.fpickerSet(j.getDate())
            }
            var f = a("#picker_" + k.id + "_h");
            if (f.length > 0) {
                f.fpickerSet(j.getHours())
            }
            var g = a("#picker_" + k.id + "_i");
            if (g.length > 0) {
                g.fpickerSet(j.getMinutes())
            }
        }
    };
    a.fn.fdtpicker = function (f) {
        var e = {
            type: "datetime",
            label: ["年", "月", "日", "时", "分"],
            format: false,
    ...
    var b = {
        init: function (c) {
            c.addClass("f-switch");
            c.html('<div class="f-switch-handle"></div>');
            c.on("tap",
                function () {
                    var d = a(this);
                    if (d.attr("readonly") || d.parents(".lr-form-row").attr("readonly")) {
                        return false
                    }
                    if (d.hasClass("f-active")) {
    ...
    var b = {
        init: function (h) {
            ...
            var d = a("#" + h.id);
            if (d.length == 0) {
                var e = '<div class="f-actionsheet f-pop" id="' + h.id + '" data-page="' + c.pageid() + '"  ></div>';
                c.createMask();
                d = a(e);
                a("body").append(d);
                var i = {};
                ...
                a.each(h.data,
                    function (j, n) {
                        var k = a.extend({
                            ...
                            n);
                        i[k.group] = i[k.group] || a('<ul class="f-table"></ul>');
                        var m = "";
                        if (!!k.mark) {
                            m = 'style="color:red;"'
                        }
                        var l = '<li class="f-table-cell"><a data-event="' + j + '" ' + m + ' href="#">' + k.text + "</a></li>";
                        i[k.group].append(l)
                    });
                for (var g in i) {
                    d.append(i[g])
                }
                d.append('<ul class="f-table"><li class="f-table-cell"><a data-event="cancel" href="#">取消</a></li></ul>');
                d.on("tap", "a", {
                    op: h
                },
                    function (k) {
    ...
    var b = {
        init: function (g) {
            ...
            if (g.type != "toast") {
                ...
                var f = '<div class="f-dialog"></div><div class="f-dialog-mask"></div>';
                e.append(f);
                b[g.type](g);
                b.bindEvent(g);
                ...
                var d = a('<div class="f-toast-container" ><div class="f-toast-message" >' + g.msg + "</div></div>");
                e.append(d);
                d.fadeIn(function () {
                    var h = d;
                    setTimeout(function () {
        ...
        warning: function (g) {
            var f = a(".f-dialog");
            var d = a('<div class="f-dialog-body" ><div class="f-dialog-title" >' + g.title + '</div><div class="f-dialog-text">' + g.msg + "</div></div>");
            var e = a('<div class="f-dialog-btns"><span data-index="0" class="f-dialog-btn f-dialog-btn-bold">' + g.btn + "</span></div>");
            f.append(d);
            f.append(e)
        },
        confirm: function (h) {
            var f = a(".f-dialog");
            var d = a('<div class="f-dialog-body" ><div class="f-dialog-title" >' + h.title + '</div><div class="f-dialog-text">' + h.msg + "</div></div>");
            var e = a('<div class="f-dialog-btns"></div>');
            var g = "";
            a.each(h.btns,
                function (i, j) {
                    g += '<span data-index="' + i + '" class="f-dialog-btn">' + j + "</span>"
                });
            e.html(g);
            f.append(d);
            f.append(e)
        ...
        prompt: function (h) {
            var f = a(".f-dialog");
            var d = a('<div class="f-dialog-body" ><div class="f-dialog-title" >' + h.title + '</div><div class="f-dialog-text">' + h.msg + '</div>            <div class="f-dialog-input" ><input type="text" placeholder="' + (h.input || "") + '"></div></div>');
            var e = a('<div class="f-dialog-btns"></div>');
            var g = "";
            a.each(h.btns,
                function (i, j) {
    ...
    c.loading = function (f, g) {
        if (f) {
            var e = "";
            if (!!g) {
                e = '<div class="f-load-msg" >' + (g || "") + "</div>"
            }
            var d = a('<div class="f-load-mask"></div><div class="f-load-container" ><div class="f-load-body" ><div style="margin-right:0px;" class="f-pull-loading f-spinner" ></div>            ' + e + "</div></div>");
            a("body").append(d);
            d.fadeIn()
        } else {
    ...
    var g = o + " " + d + " " + e;
    var f = o + " " + d + " " + q;
    var u = ['<div class="' + i + '">', '<div class="{icon}"></div>', '<div class="' + k + '">{contentrefresh}</div>', "</div>"].join("");
    var v = {
        init: function (w, x) {
    ...
    var f = e.Scroll.extend(a.extend({
        ...
        _start: function (i) {
            if (i.touches && i.touches.length && i.touches[0].clientX > 30) {
                i.target && !this._preventDefaultException(i.target, this.options.preventDefaultException) && e.preventDefault(i)
            }
            if (!this.loading) {
        ...
        resetPosition: function (i) {
            if (this.pulldown) {
                if (this.y >= this.options.down.height) {
                    ...
                    !this.loading && this.topPocket.classList.remove(c)
                }
            }
    ...
    var o = k.Scroll.extend({
        init: function (r, v) {
            v = v || {};
            r.addClass(e);
            var s = a('<div class="f-slider-group" ></div>');
            r.append(s);
            var w = this;
            if (v.loop == p) {
                w.loop = true
            ...
            if (u == 0) {
                ...
                if (u == 1) {
                    s.append('<div class="' + h + '"><div><img src="' + t[0] + '"></div></div>')
                } else {
                    if (w.loop) {
                        s.append('<div class="' + h + ' f-slider-item-duplicate"><div><img src="' + t[u - 1] + '"></div></div>')
                    }
                    a.each(t,
                        function (x, y) {
                            if (!!v.indicator) {
                                w.$indicator.append('<div class="f-indicator ' + (x == 0 ? d : "") + '" ></div>')
                            }
                            s.append('<div class="' + h + " " + (x == 0 ? d : "") + ' "><div><img src="' + y + '"></div></div>')
                        });
                    if (w.loop) {
                        s.append('<div class="' + h + ' f-slider-item-duplicate"><div><img src="' + t[0] + '"></div></div>')
                    }
                }
            }
    ...
    var b = {
        init: function (g) {
            ...
            if (f.length == 0) {
                f = a('<div id="fui_imagePreview"  class="f-imagePreview" ><div class="f-imagePreview-slider" ></div></div>');
                a("body").append(f);
                b.bindEvent(f)
            }
        ...
        close: function (f) {
            ...
            setTimeout(function () {
                ...
                f.append('<div class="f-imagePreview-slider" ></div>');
                g = null;
                f = null
            },
                500)
        },
        laterCloseEvent: function () {
            !e && (e = d.later(function () {
    ...
    a.fn.ftimeline = function (f) {
        ...
        var e = a('<div class="f-timeline-allwrap"></div>');
        var d = a("<ul></ul>");
        var b = a('<li class="f-timeline-header"><div>当前</div></li>');
        d.append(b);
        a.each(f,
            function (j, k) {
                var i = a('<li class="f-timeline-item" ><div class="f-timeline-wrap" ></div></li>');
                if (j == 0) {
                    i.find("div").addClass("f-timeline-current")
                }
                var h = i.find(".f-timeline-wrap");
                var g = a('<div class="f-timeline-content"><span class="arrow"></span></div>');
                g.append('<div class="f-timeline-title">' + k.title + "</div>");
                g.append('<div class="f-timeline-body"><span>' + k.people + "</span>" + k.content + "</div>");
                h.append('<span class="f-timeline-date">' + k.time + "</span>");
                h.append(g);
                d.append(i)
            });
        d.append('<li class="f-timeline-ender"><div>开始</div></li>');
        e.html(d);
        c.html(e)
        c.scroll()
    }
    ...
    var b = {
        init: function (d, g) {
            var f = '            <div class="f-popright" >                <div class="f-popright-content">                <div class="f-popright-title" ><div class="f-popright-btn-left">';
            if (g.cancelBtn) {
                f += '<div class="f-popright-btn cancel" data-value="cancel" >' + g.cancelBtn + "</div>"
            }
            if (g.restBtn) {
                f += '<div class="f-popright-btn rest" data-value="rest">' + g.restBtn + "</div>"
            }
            f += "</div>";
            if (g.okBtn) {
                f += '<div class="f-popright-btn ok" data-value="ok">' + g.okBtn + "</div>"
            }
            f += '</div>                <div class="f-popright-body" ></div>            </div>        </div>';
            var e = a(f);
            d.parent().append(e);
    ...
    var b = {
        init: function (d, f) {
            f.id = c.guid();
            var e = '<div class="f-checkbox f-pop" id="pop_' + f.id + '"  data-page="' + c.pageid() + '"  >';
            e += '<div class="f-checkbox-header" >';
            e += '<div class="f-checkbox-cancel">取消</div>';
            e += '<div class="f-checkbox-ok">确定</div>';
            e += "</div>";
            e += '<div class="f-checkbox-body" >';
            f.dataMap = {};
            a.each(f.data,
                function (g, h) {
                    f.dataMap[h[f.ivalue]] = h;
                    e += '<div class="f-checkbox-item" data-value="' + h[f.ivalue] + '" >' + h[f.itext] + "</div>"
                });
            e += "</div></div>";
            a("body").append(e);
            c.createMask();
            ...
            if (g) {
                ...
                a.each(g.data,
                    function (h, i) {
                        ...
                        d.append('<div class="f-checkbox-item" data-value="' + i[g.ivalue] + '" >' + i[g.itext] + "</div>")
                    });
                d = null
            }
        }
...
})(window.jQuery, window.fui);
/*
 * 版 本 Learun-Mobile V2.0.0 东雅图敏捷开发框架(http://www.dongyatu.com)
...
(function ($, fui, window) {
    ...
    window.lrmui = {// 东雅图mobileui框架基础方法
        isreal: false,
        isready: false,
...
(function ($, learun, fui, window) {
    ...
    learun.http = {
        ...
        cordovaget: function (url, param, callback, dataType) {
            url = encodeURI(url);
...
(function ($, learun, fui, window) {
    ...
    learun.nav = {
        ...
        go: function (op) {
            ...
            $.extend(dfop, op || {});
            if (dfop.path === '') {
                fui.dialog({ msg: '东雅图信息提示：不知道页面路径！' });
                return false;
            }
    ...
    learun.code = {
        scan: function (callback) {
            if (learun.isreal) {
                ...
                learun.layer.warning('浏览器环境不支持扫描', null, '系统提示', '好的');
            }
        },
        encode: function (op) {// id:div的id，text：需要生成的文本；width：宽度；height：高度；colorDark，colorLight；
            var qrcode = new QRCode(document.getElementById(op.id), {
                text: op.text || "http://www.dongyatu.com/",
                width: op.width || 128,
 ...
 * 创建人：东雅图-前端开发组
 * 日 期：2018.01.08
 * 描 述：东雅图移动端框架 图片选择插件（支持拍照和从相册中选择）
 */
(function ($, learun, fui, window) {
    "use strict";
    var _ft = null;
    ...
    learun.imagePick = function (callback, op) {
        if (learun.isreal) {
            ...
            learun.layer.warning('浏览器环境不支持图片选择', null, '系统提示', '好的');
        }
    };
    // 拍照获取图片
    learun.camera = function (callback) {
        ...
        if (true) {
            ...
            //    });
            $('<input type="file" accept="image/*"/>').append('body').click().change(function () {
            ...
            learun.layer.warning('浏览器环境不支持拍照', null, '系统提示', '好的');
        }
    };
    ...
    var _imagepicker = {
        ...
        upload: function (url, op, callback) {// 上传图片文件
            ...
            if (op.uploadUrl) {
                ...
                $.ajax({
                    url: op.uploadUrl + '?token=' + op.params.token + '&loginMark=' + op.params.loginMark + '&data=' + op.params.data,
                    type: 'POST',
                    data: url,
                    processData: false,
                    contentType: false,
                    ...
                    success: function (responseStr) {
                        var _res = responseStr
                        if (_res.code === 200) {
                            callback(true, op.downUrl + _res.data, op, _res.data)
                        }
                        else {
                            callback(false, op.downUrl + _res.data, op);
                        }
                    },

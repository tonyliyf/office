...
(function ($, learun) {
    ...
    $.fn.lrGetFormData = function (keyValue) {// 获取表单数据
        var resdata = {};
        $(this).find('input,select,textarea,.lr-select,.lr-formselect,.lrUploader-wrap,.lr-radio,.lr-checkbox,.edui-default,.lr-select1').each(function (r) {
            var id = $(this).attr('id');
            if (!!id) {
                ...
                switch (type) {
                    ...
                    case "lrselect":
                        ...
                        break;
                    case "lrselect1":
                        resdata[id] = $(this).find('input.lr-select-placeholder').val();
                        break;
                    case "formselect":
                    ...
                    case "lr-radio":
                        ...
                        break;
                        //if (id.indexOf('_easyui') < 0) {
                        //    resdata[id] = $(this).find('#_' + id).combobox('getValue')
                        //}
                        break;
                    case "lr-checkbox":
                        //var _idlist = [];
                        ...
                        if (id.indexOf('_easyui') < 0) {
                            resdata[id] = ''
                            $.each($(this).find('#_' + id).combobox('getValues'), function (i, v) {
                                resdata[id] += v + ','
                            })
                            resdata[id] = resdata[id].substr(0, resdata[id].length - 1)
                        }
                        break;
                    default:
    ...
    $.fn.lrSetFormData = function (data) {// 设置表单数据         
        ...
        for (var id in data) {
            ...
            //if (n>1 &&id.lastIndexOf("_") != -1) {

            //    value = data[id.substring(0, id.lastIndexOf("_"))];
            //}
            //else {
            //    value = data[id];
            //}
            ...
            if ($obj.length == 0 && value != null) {
                ...
                switch (type) {
                    case "lr-checkbox":
                        var cbArr = value.split(',')
                        if (id.indexOf('_easyui') < 0) {
                            $obj.find('#_' + id).combobox('setValues', cbArr)
                        }
                        break;
                    case "checkbox":
                        var isck = 0;
                    ...
                    case "lrselect":
                        ...
                        break;
                    case "lrselect1":
                        $obj.find('input.lr-select-placeholder').val(value);
                        break;
                    case "formselect":
                    ...
                    case "lr-radio":
                        ...
                        //if (id.indexOf('_easyui') < 0) {
                        //    $obj.find('#_' + id).combobox('setValues', value)
                        //}
                        break;
                    default:
    ...
    $.fn.lrRadioCheckbox = function (op) {
        ...
        var compont1 = '<select id="_' + thisId + '" class="easyui-combobox" type="mycombobox"'
        var compont2 = ' style = "width:100%" ></select >'
        var initData = []
        if (dfop.dataType == 'dataItem') {
                ...
                callback: function (dataes) {
                    if (dfop.type == 'radio') {
                        $.each(dataes, function (id, item) {
                            var $point = $('<label><input name="' + thisId + '" value="' + item.value + '"' + (readonly ? 'disabled="disabled"' : '') + ' type="' + dfop.type + '">' + item.text + '</label>');
                            $this.append($point);
                        });
                    } else {
                        $.each(dataes, function (id, item) {
                        ...
                        var dataOptions = 'data-options=\'editable:false,' + (readonly ? 'disabled:true,' : '') + 'valueField: "key",multiple:true,height:26,textField: "value",onChange:function(){$(this).parent().removeClass("lr-field-error").parent().find(".lr-field-error-info").remove()},data:' + JSON.stringify(initData) + '\''
                        var $compont = $(compont1 + dataOptions + compont2)
                        $this.append($compont)
                        $.parser.parse()
                    }
                }
            ...
            learun.clientdata.getAllAsync('sourceData', {
                ...
                callback: function (dataes) {
                    if (dfop.type == 'radio') {
                        $.each(dataes, function (id, item) {
                            var $point = $('<label><input name="' + thisId + '" value="' + item[dfop.value] + '" ' + (readonly ? 'disabled="disabled"' : '') + '' + '" type="' + dfop.type + '">' + item[dfop.text] + '</label>');
                            $this.append($point);
                        });
                    } else {
                        $.each(dataes, function (id, item) {

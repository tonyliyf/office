...
var bootstrap = function ($, learun) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#lr_add').on('click', function () {
                learun.layerForm({
                    id: 'form',
                    title: '新增',
                    url: top.$.rootUrl + '/LR_CodeDemo/DC_OA_OverSeeWorkClosing/Form',
                    width: 600,
                    height: 400,
            ...
            $('#lr_edit').on('click', function () {
                var keyValue = $('#gridtable').jfGridValue('F_OSWCId');
                if (learun.checkrow(keyValue)) {
                    learun.layerForm({
                        id: 'form',
                        title: '编辑',
                        url: top.$.rootUrl + '/LR_CodeDemo/DC_OA_OverSeeWorkClosing/Form?keyValue=' + keyValue,
                        width: 600,
                        height: 400,
                        callBack: function (id) {
                            var res = false;
            ...
            $('#lr_delete').on('click', function () {
                ...
                if (learun.checkrow(keyValue)) {
                    learun.layerConfirm('是否确认删除该项！', function (res) {
                        if (res) {
                            learun.deleteForm(top.$.rootUrl + '/LR_CodeDemo/DC_OA_OverSeeWorkClosing/DeleteForm', { keyValue: keyValue}, function () {
                                refreshGirdData();
                            });
                        }
                    });
        ...
        initGird: function () {
            $('#gridtable').jfGrid({
                url: top.$.rootUrl + '/LR_CodeDemo/DC_OA_OverSeeWorkClosing/GetPageList',
                headData: [
                    ...
                    { label: "申请时间", name: "F_EndCloseDate", width: 100, align: "left"},
                    { label: "附件上传", name: "F_Attachments", width: 100, align: "left"},
                    { label: "申请说明", name: "F_CloseExplain", width: 100, align: "left"},

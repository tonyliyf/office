...
(function () {
    var begin = '';
    var end = '';
    learun.storage.set('CurrentAuditPage', 'nworkflow/mycreate')
    var $header = null;
    ...
    var page = {
        ...
        init: function ($page) {
            ...
            $header.find('.lr-form-header-more').on('tap', function () {
                learun.nav.go({ path: 'search', title: '', isBack: true, isHead: true, param: 'nworkflow/mycreate' });// 告诉搜索页本身所在的地址
            });
            $page.find('#mycreate_tab').toptab(['我的', '已办'], function (_index) {
            ...
            //var s = $page.find('#mycreate_tab')
            //s = s.find('div.f-toptab-btns')
            //s = s.find('div')
            //s = s.find('a[data-value="0"]')
            //s.tap()
        },
        lclass: 'lr-list lr-flow-list',
        loadData: function (param, callback, $page) {// 列表加载后台数据
            ...
            var url = '';
            var mypage = learun.nav.getpage('nworkflow/mycreate');
            switch (mypage.currentPage) {
                case 0:
                    url = config.webapi + "learun/adms/newwf/mylist?type=0";
                    break;
                //case 1:
                //    url = config.webapi + "learun/adms/newwf/mytask";
                //    break;
                case 1:
                    url = config.webapi + "learun/adms/newwf/mylist?type=1";
                    break;
            }

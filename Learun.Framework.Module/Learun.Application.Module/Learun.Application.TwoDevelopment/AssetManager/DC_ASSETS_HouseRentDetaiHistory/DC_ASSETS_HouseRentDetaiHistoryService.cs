...
namespace Learun.Application.TwoDevelopment.AssetManager
{
    ...
    /// </summary>
    public class DC_ASSETS_HouseRentDetaiHistoryService : RepositoryFactory
    {
        #region 获取数据
        ...
        /// <param name="queryJson">查询参数</param>
        /// <returns></returns>
        public IEnumerable<DC_ASSETS_HouseRentDetaiHistoryEntity> GetRentPageList(Pagination pagination, string queryJson)
        {
            try
            {
                var strSql = new StringBuilder();
                ...
                return this.BaseRepository().FindList<DC_ASSETS_HouseRentDetaiHistoryEntity>(strSql.ToString(), dp, pagination);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        /// <param name="queryJson">查询参数</param>
        /// <returns></returns>
        public IEnumerable<DC_ASSETS_HouseRentDetailEntity> GetRentDetailinfo( string keyValue,string SearchValue)
        {
            try
            {
                var strSql = new StringBuilder();
                ...
                strSql.Append("select m.F_RentName, d.*,m.F_Unit as F_FormerUnit from  DC_ASSETS_HouseRentDetailHistory d join  dc_assets_houseRentMainHistory m on m.F_HRMId =d.F_HRMId ");
...
select m.F_RentName, d.F_Transferor,d.F_Location,h.F_CertificateNo,d.F_RentArea,d.F_RentReservePrice,d.F_RentDeposit
 ...
 ,i.F_Renter as Renter,i.F_RenterPhone as phone,i.F_DoThings ,i.F_RentArea as Area,i.F_ContractNo as ContractNo,i.F_RentStartTime as starttime ,i.F_RentAge as age,i.F_ActualPrice as price,i.F_RentDeposit as Deposit,i.F_Manager,i.F_Remarks as marks from  dc_assets_houseRentDetail d join  dc_assets_houseRentMain m on m.F_HRMId =d.F_HRMId join DC_ASSETS_HouseInfo h 
        ...
        /// <summary>
        /// <returns></returns>
        public DC_ASSETS_HouseRentDetaiHistoryEntity GetHouseRentDetailEntity(string keyValue)
        {
            try
            {
                return this.BaseRepository().FindEntity<DC_ASSETS_HouseRentDetaiHistoryEntity>(keyValue);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        /// <summary>
        /// <returns></returns>
        public IEnumerable<DC_ASSETS_HouseRentDetail_InfoHistoryEntity> GetDC_ASSETS_HouseRentDetailInfoList(string keyValue)
        {
            try
            {
                string sql = @"  select  * 
                ...
                return this.BaseRepository().FindList<DC_ASSETS_HouseRentDetail_InfoHistoryEntity>(sql, new { F_RentDetailId = keyValue });
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        public DataTable GetSqlTree()
        {
            try
            {
                return this.BaseRepository().FindTable(@"  select * from (select distinct F_RentYear as id ,0 as parentid,F_RentYear as [name] ,0 as F_RentNumber from DC_ASSETS_HouseRentMainHistory
  union select F_HRMId as id ,F_RentYear as parentid,F_RentName as [name],F_RentNumber  from DC_ASSETS_HouseRentMainHistory) b order by parentid,cast(F_RentNumber as int) ");
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        /// <summary>
        /// <returns></returns>
        public void SaveEntity(string keyValue, DC_ASSETS_HouseRentDetaiHistoryEntity entity)
        {
            try
            {

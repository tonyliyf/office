...
var bootstrap = function ($, learun) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#F_OSWId').lrDataSourceSelect({
                code: 'OSTaskUncompleteData',
                value: 'f_oswid',
                text: 'f_oswcontent',
                select: function (rowData) {
                    if (rowData && parent.location.search.indexOf('type=create') >= 0) {
                        learun.loading(true, '正在获取数据')
                        $.post(top.$.rootUrl + '/LR_CodeDemo/DC_OA_OverSeeWorkClosing/GetInitData',
                            { keyValue: rowData.f_oswid },
                            function (res) {
                                learun.loading(false)
                                if (res.code == 200) {
                                    $('#DC_OA_OverSeeWorkClosingDetailed').jfGridSet('refreshdata', res.data.DC_OA_OverSeeWorkClosingDetailed);
                                    delete res.data.DC_OA_OverSeeWorkClosing.F_OSWId
                                    $('[data-table="DC_OA_OverSeeWorkClosing"]').lrSetFormData(res.data.DC_OA_OverSeeWorkClosing);
                                } else {
            ...
            $('#DC_OA_OverSeeWorkClosingDetailed').jfGrid({
                headData: [
                    {
        ...
        initData: function () {
            if (!!keyValue) {
                $.lrSetForm(top.$.rootUrl + '/LR_CodeDemo/DC_OA_OverSeeWorkClosing/GetFormData?keyValue=' + keyValue, function (data) {
                    for (var id in data) {
    ...
    setFormData = function (processId) {
        if (!!processId) {
            $.lrSetForm(top.$.rootUrl + '/LR_CodeDemo/DC_OA_OverSeeWorkClosing/GetFormDataByProcessId?processId=' + processId, function (data) {
                for (var id in data) {
                    if (!!data[id] && data[id].length > 0) {
                        $('#' + id).jfGridSet('refreshdata', data[id]);
                    }
                    else {
                        if (id == 'DC_OA_OverSeeWorkClosing' && data[id]) {
                            keyValue = data[id].F_OSWCId;
                        }
                        $('[data-table="' + id + '"]').lrSetFormData(data[id]);
                    }
                }
    ...
    save = function (processId, callBack, i) {
        var postData = {};
        var formData = $('[data-table="DC_OA_OverSeeWorkClosing"]').lrGetFormData();
        if (!!processId) {
            formData.F_OSWCId = processId;
        }
        postData.strEntity = JSON.stringify(formData);
        postData.strdC_OA_OverSeeWorkClosingDetailedList = JSON.stringify($('#DC_OA_OverSeeWorkClosingDetailed').jfGridGet('rowdatas'));
        $.lrSaveForm(top.$.rootUrl + '/LR_CodeDemo/DC_OA_OverSeeWorkClosing/SaveForm?keyValue=' + keyValue, postData, function (res) {
            // 保存成功后才回调
            if (!!callBack) {
                callBack(res, i);
            }

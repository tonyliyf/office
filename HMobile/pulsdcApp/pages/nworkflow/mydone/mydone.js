...
(function () {
    ...
    var end = '';
    var $header = null;
    learun.storage.set('CurrentAuditPage', 'nworkflow/mydone')
    var page = {
        currentPage: 0,
        grid: [],
        init: function ($page) {
            ...
            $header.find('.lr-form-header-more').on('tap', function () {
                learun.nav.go({ path: 'search', title: '', isBack: true, isHead: true, param: 'nworkflow/mydone' });// 告诉搜索页本身所在的地址
            });
            $page.find('#mydone_tab').toptab(['我的', '待办', '已办'], function (_index) {
            ...
            var s = $page.find('#mydone_tab')
            s = s.find('div.f-toptab-btns')
            s = s.find('div')
            s = s.find('a[data-value="2"]')
            s.tap()
        },
        lclass: 'lr-list lr-flow-list',
        loadData: function (param, callback, $page) {// 列表加载后台数据
            ...
            var url = '';
            var mypage = learun.nav.getpage('nworkflow/mydone');

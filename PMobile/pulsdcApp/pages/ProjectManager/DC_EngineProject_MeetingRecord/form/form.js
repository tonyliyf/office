    ...
    var page = {
        ...
        init: function ($page, param) {
                            ...
                            event: function () {// 删除当前条信息
                                learun.layer.confirm('确定要删除该笔数据吗？', function (_index) {
                                    if (_index === '1') {
                                        ...
                                        learun.httppost(config.webapi + 'learun/adms/ProjectManager/DC_EngineProject_MeetingRecord/delete', keyValue, (data) => {
                                            learun.layer.loading(false);
                                            if (data) {// 删除数据成功
                                                learun.nav.closeCurrent();
                                                var prepage = learun.nav.getpage('ProjectManager/DC_EngineProject_MeetingRecord');
                                                prepage.grid.reload();
                                            }
                                        });
                                    }
            ...
            $header.find('.lr-form-header-submit').on('tap', function () {
                ...
                learun.httppost(config.webapi + 'learun/adms/ProjectManager/DC_EngineProject_MeetingRecord/save', _postData, (data) => {
                    learun.layer.loading(false);
                    if (data) {// 表单数据保存成功
                        if (keyValue) {
            ...
            if (keyValue) {
                ...
                learun.httpget(config.webapi + 'learun/adms/ProjectManager/DC_EngineProject_MeetingRecord/form', keyValue, (data) => {
                    if (data) {
                        for (var id in data) {
                            if (data[id].length) {
        ...
        bind: function ($page, param) {
            if (!keyValue) {
                learun.getRuleCode('', function (data) {
                    $page.find('#F_MRNum').val(data);
                });
            }
            $page.find('#F_ConveningDepartment').lrselect({
                type: 'user',
                needPre:false
            });
            $page.find('#F_Convenor').lrselect({
                type: 'user',
                needPre:false
            });
            $page.find('#F_MRType').lrpickerex({
            code: 'ConventionNum',
            type: 'dataItem'            });
            $page.find('#F_StartTime').lrdate({
                type: 'date'
            });
            $page.find('#F_EndTime').lrdate({
                type: 'date'
            });
            ...
            if (!keyValue) {
            $page.find('#F_CreateDatetime').val(learun.date.format(new Date(), 'yyyy-MM-dd hh:mm:ss'));
            }

...
var bootstrap = function ($, learun) {
    ...
    var page = {
        init: function () {
            ...
            $('#btn_area').click(function () {
                learun.layerForm({
                    id: 'areaform',
                    title: '选择区域',
                    url: top.$.rootUrl + '/AssetManager/DC_ASSETS_LandBaseInfo/MapForm?keyValue=' + $('#F_LBIId').lrselectGet(),
                    width: 600,
                    height: 400,
        ...
        bind: function () {
            ...
            $('#F_LBIId').lrDataSourceSelect({
                code: 'GetLandbaseInfo', value: 'f_lbiid', text: 'landname',
                ...
                select: function (rowData) {
                    if (rowData) {
                        ...
                        $.lrSetForm(top.$.rootUrl + '/AssetManager/DC_ASSETS_LandBaseInfo/GetFormData?keyValue=' + rowData.f_lbiid, function (data) {
                            for (var id in data) {
        ...
        initData: function () {
            if (!!keyValue) {
                $.lrSetForm(top.$.rootUrl + '/AssetManager/DC_ASSETS_HouseInfo/GetTotalHouseInfo?keyValue=' + keyValue, function (data) {
                    for (var id in data) {
                        if (!!data[id].length && data[id].length > 0) {
                            $('#' + id).jfGridSet('refreshdata', data[id]);
                        }
    ...
    acceptClick = function (callBack) {
        ...
        var strLandEntityss = $('[data-table="DC_ASSETS_LandBaseInfo"]').lrGetFormData();
        ...
        $.lrSaveForm(top.$.rootUrl + '/AssetManager/DC_ASSETS_HouseInfo/SaveTotalForm?keyValue=' + keyValue, postData, function (res) {
            // 保存成功后才回调
            if (!!callBack) {
                callBack();
            }

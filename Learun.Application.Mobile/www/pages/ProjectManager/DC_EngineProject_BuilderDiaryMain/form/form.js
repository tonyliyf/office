    ...
    var page = {
        ...
        init: function ($page, param) {
                            ...
                            event: function () {// 删除当前条信息 
                                learun.layer.confirm('确定要删除该笔数据吗？', function (_index) {
                                    if (_index === '1') {
                                        ...
                                        learun.httppost(config.webapi + 'learun/adms/ProjectManager/DC_EngineProject_BuilderDiaryMain/delete', keyValue, (data) => {
                                            learun.layer.loading(false);
                                            if (data) {// 删除数据成功 
                                                learun.nav.closeCurrent();
                                                var prepage = learun.nav.getpage('ProjectManager/DC_EngineProject_BuilderDiaryMain');
                                                prepage.grid.reload();
                                            }
                                        });
                                    }
            ...
            $header.find('.lr-form-header-submit').on('tap', function () {
                ...
                _postData.strEntity = JSON.stringify($page.find('[data-table="DC_EngineProject_BuilderDiaryMain"]').lrformGet());
                _postData.strdC_EngineProject_BuilderDiaryDetailList = JSON.stringify($page.find('#DC_EngineProject_BuilderDiaryDetail').lrgridGet());
                learun.layer.loading(true, '正在提交数据');
                learun.httppost(config.webapi + 'learun/adms/ProjectManager/DC_EngineProject_BuilderDiaryMain/save', _postData, (data) => {
                    learun.layer.loading(false);
                    if (data) {// 表单数据保存成功 
                        if (keyValue) {
            ...
            if (keyValue) {
                ...
                learun.httpget(config.webapi + 'learun/adms/ProjectManager/DC_EngineProject_BuilderDiaryMain/form', keyValue, (data) => {
                    if (data) {
                        for (var id in data) {
                            if (data[id].length) {
        ...
        bind: function ($page, param) {
            ...
            if (!keyValue) {
                learun.getRuleCode('systemNum', function (data) {
                    $page.find('#F_BDNum').val(data);
                });
            }
            $page.find('#F_CreateDepartmentId').lrselect({
                type: 'department'
            });
            if (!keyValue) {
                $page.find('#F_CreateDepartmentId').lrselectSet(learun.storage.get('userinfo').baseinfo.departmentId);
            }
            $page.find('#F_CreateUserid').lrselect({
                type: 'user'
            });
            if (!keyValue) {
                $page.find('#F_CreateUserid').lrselectSet(learun.storage.get('userinfo').baseinfo.userId);
            }
            $page.find('#F_CreateDatetime').lrdate({
                type: 'date'
            });
            $page.find('#F_Attachments').imagepicker();
            $page.find('#DC_EngineProject_BuilderDiaryDetail').lrgrid({
                title: '编辑表格',
                componts: [
                    { name: '施工班组', field: 'F_WorkTeam', type: 'input' },
                    { name: '施工人数', field: 'F_Workers', type: 'input' },
                    { name: '施工内容', field: 'F_WorkContent', type: 'input' },
                ]
            });
